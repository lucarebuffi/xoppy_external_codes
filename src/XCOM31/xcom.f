...
C
Csrio      CHARACTER PSOR*30,INPUT*10,XFIL1*30,XFIL2*30,SUB*72,
Csrio     1 IND(100)*3,FF*1
      CHARACTER PSOR*120,INPUT*124,XFIL1*60,XFIL2*60,SUB*72,
     1 IND(100)*3,FF*1
...
Csrio      DO 13 J=30,1,-1
  ...
  622 CONTINUE
      ...
      NN=1
      JTAL=0
Csrio      DO 880 N=1,NENG
Csrio      IF(JTAL)640,640,805
Csrio  640 IF(N-1)650,650,645
Csrio  645 WRITE (8,*) FF
  650 WRITE (8,660) SUB
  660 FORMAT(8X,A)
      WRITE (8,10)
      WRITE (8,662)
  662 FORMAT(8X,'Constituents (Atomic Number:Fraction by Weight)')
Csrio      WRITE (8,665) (NZ(K),WEIGHT(K),K=1,KMAX)
Csrio  665 FORMAT((8X,6(1X,I3,':',F7.5)))                                    SMS
      WRITE (8,665) '#',(NZ(K),WEIGHT(K),K=1,KMAX)
  665 FORMAT((7X,A,6(1X,I3,':',F7.5)))                                    SMS
      WRITE (8,10)
...
C 792 FORMAT(9X,'(MeV)',3x,7('(b/atom) '))
  ...
  800 WRITE (8,10)
C srio makes the do start here
      DO 880 N=1,NENG
C srio makes the do start here
...
C 820 FORMAT(7X,1PE10.3,1P7E9.2)
  820 FORMAT(7X,1PE10.3,1P7E10.3)                                       SMS
      ...
      NN=NN+1
C Changed by srio
Csrio      WRITE(8,840) KZ(N),ALAB(N),ENN,SCTCO(N),SCTIN(N),PHT(N),
Csrio     1 PRAT(N),PREL(N),AT(N),ATNC(N)
      WRITE(8,841) ALAB(N),KZ(N)
      WRITE(8,820) ENN,SCTCO(N),SCTIN(N),PHT(N),
     1 PRAT(N),PREL(N),AT(N),ATNC(N)
  841 FORMAT(1X,A2,1X,I2)
C 840 FORMAT(1X,I2,1X,A2,1X,1PE10.3,1P7E9.2)
Csrio  840 FORMAT(I3,1X,A2,1X,1PE10.3,1P7E10.3)                              SMS
      X(NN,1)=ENN
      X(NN,2)=SCTCO(N)
      X(NN,3)=SCTIN(N)
      X(NN,4)=PHT(N)
...
C added by srio@esrf.eu
   ...
   10 FORMAT(1H )
   12 FORMAT(A)
csrio ask for data directory (for Xop use)
        write(*,*)'Xcom will read the datafiles from a data directory.'
        write(*,*) 
     1   'Enter the data directorty path with the last delimiter'
        write(*,*)' [slash in Unix or backslash in Windows95]: '
        read(*,'(a)') datadir
...
C     RETURN
Csrio  243 SOURCE='MDATX3.'
  243 SOURCE=datadir(1:iblank(datadir)) // 'MDATX3.'
      RETURN
      END
...
C function added for being used with xop
