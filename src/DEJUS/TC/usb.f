...
C ----------------+-------+-----------------------------------------------------
C 25-NOV-2014     | RJD   | Fixed error in SUBROUTINE CONVOLUTE_ENERGY_VSTEP. The array HE() was
C                 |       | accessed with index 0 (out of bounds for J2 = 1). See RJD 10/23/2014.
C                 |       | Fix has no effect on the results because the calculated spectra SP1 would
C                 |       | typically be zero at the boundary. Increased default values of NALPHA and NSIGMA.
C                 |       | Current version is v1.21.
C ----------------+-------+-----------------------------------------------------
C-
C  Size parameters:
...
C RJD added check 10/23/2014
C	      SUM = SUM+SP1(J2)*SINC(ARG)*(H2+HE(J2-1)/TWO)
C	      DO IE=1,J2-1
C	        ARG = PE*(EUP-E(IE))
C	        SUM = SUM+SP1(IE)*SINC(ARG)*HA(IE)
C	      END DO ! IE
	      IF (J2 .EQ. 1) THEN
		  SUM = SUM+SP1(J2)*SINC(ARG)*H2/TWO
	      ELSE ! J2 > 1
